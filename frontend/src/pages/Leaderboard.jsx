...
function TeamsLeaderboard() {
  ...
  const [sortBy, setSortBy] = useState('locked') // locked | backers

  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <div className="text-sm text-stadium-muted">
          Total alive locked: <span className="text-white font-semibold">${formatUSDC(totalAlive)}</span>
        </div>
        <div className="flex gap-2">
          <button
            onClick={() => setSortBy('locked')}
            className={`text-xs px-3 py-1 rounded-full transition-colors ${sortBy === 'locked' ? 'bg-stadium-green text-stadium-dark' : 'text-stadium-muted border border-stadium-border'}`}
          >
            By Locked
          </button>
          <button
            onClick={() => setSortBy('backers')}
            className={`text-xs px-3 py-1 rounded-full transition-colors ${sortBy === 'backers' ? 'bg-stadium-green text-stadium-dark' : 'text-stadium-muted border border-stadium-border'}`}
          >
            By Backers
          </button>
        </div>
  ...
  return (
    <div className={`card flex items-center gap-4 py-3 ${eliminated ? 'opacity-40' : ''}`}>
      ...
      <div className="flex-1">
        ...
        <div className="w-full bg-stadium-border rounded-full h-1.5">
          <div
            className={`h-1.5 rounded-full transition-all ${eliminated ? 'bg-stadium-muted' : 'bg-stadium-green'}`}
            style={{ width: `${Math.min(pct, 100)}%` }}
          />
        </div>
      </div>
      <div className="text-right">
        <div className="font-semibold text-white text-sm">${formatUSDC(locked)}</div>
...
function ChampionPoolView() {
  ...
  const CONTRIBUTIONS = [
    ...
    { source: 'VAR Losing Bets', pct: '25%', desc: '25% of each losing VAR bet\'s 90% redistribution' },
  ]
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="card border-stadium-gold/30 bg-stadium-gold/5 text-center py-8">
        ...
        {champion && (
          <div className="mt-4 badge-gold text-sm mx-auto w-fit">
            Champion: {champion}
          </div>
        )}
      </div>
      ...
      {/* How it accumulates */}
      <div className="card">
        <h2 className="font-semibold text-white mb-4">How the Pool Grows</h2>
        <div className="space-y-3">
          {CONTRIBUTIONS.map(c => (
            <div key={c.source} className="flex items-start gap-4 p-3 bg-stadium-dark rounded-xl">
              <div className="text-2xl font-bold text-stadium-gold font-mono w-12">{c.pct}</div>
              <div>
                <div className="font-medium text-white text-sm">{c.source}</div>
      ...
      {/* Distribution */}
      <div className="card">
        <h2 className="font-semibold text-white mb-4">Distribution at Tournament End</h2>
        <div className="text-stadium-muted text-sm space-y-3">
          ...
          <div className="bg-stadium-dark rounded-xl p-4 font-mono text-xs text-stadium-green border border-stadium-border">
      ...
      {/* Top Team Preview */}
      <div className="card">
        <h2 className="font-semibold text-white mb-4">Tournament Favorites</h2>
        <div className="space-y-2">
...
function FavoriteRow({ team }) {
  ...
  return (
    <div className={`flex items-center gap-3 p-3 rounded-xl ${eliminated ? 'opacity-30' : 'bg-stadium-dark'}`}>
      ...
      <div className="text-right text-xs text-stadium-muted">
        <div className="text-white font-medium">${formatUSDC(locked)}</div>
