    ...
    <div className="space-y-8">
      ...
      <div className="card border-blue-500/20 bg-blue-500/5">
        <div className="grid md:grid-cols-3 gap-6 text-sm">
          <div>
            <div className="font-semibold text-white mb-2">How VAR Pays Out</div>
            <div className="text-stadium-muted space-y-1">
              <div>✓ Correct: your bet + share of losing pool</div>
              <div>✓ CONVICTION holder: 1.5× net winnings</div>
              <div>✓ Wrong: 10% of your bet returned automatically</div>
            </div>
          </div>
          <div>
            <div className="font-semibold text-white mb-2">VAR Window</div>
            <div className="text-stadium-muted space-y-1">
          ...
          <div>
            <div className="font-semibold text-white mb-2">Loss Distribution</div>
            <div className="text-stadium-muted space-y-1">
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1">
          <h2 className="font-semibold text-white mb-4">Live Matches</h2>
...
function MatchListItem({ matchId, isSelected, onSelect }) {
  ...
  const statusText = match.settled ? 'Settled' : match.varClosed ? 'In Progress' : match.varOpen ? '🟢 VAR Open' : 'Upcoming'
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between mb-1">
        ...
        <span className={`text-xs ${match.varOpen ? 'text-stadium-green' : 'text-stadium-muted'}`}>
          {statusText}
        </span>
      </div>
...
function VARBettingPanel({
  ...
  const totalPool = market ? (market.totalYesPool + market.totalNoPool) : 0n
  ...
  return (
    <div className="card space-y-5">
      ...
      <div className="text-center">
        ...
        {hasBonus && (
          <div className="badge-gold text-xs mx-auto w-fit">
            ⚡ 1.5× CONVICTION bonus active
          </div>
        )}
        {match?.varOpen ? (
      ...
      <div className="grid grid-cols-4 gap-2">
        {MARKET_NAMES.map((name, i) => (
          <button
            ...
            className={`text-center p-2.5 rounded-lg border transition-colors text-xs ${
              selectedMarket === i
          ...
          >
            <div className="text-lg mb-1">{MARKET_ICONS[i]}</div>
      ...
      {/* Pool Info */}
      <div className="flex justify-between text-xs text-stadium-muted bg-stadium-dark rounded-lg p-3">
        <span>Total pool: ${formatUSDC(totalPool)}</span>
        <span>Yes pool: ${formatUSDC(market?.totalYesPool)}</span>
        <span>No pool: ${formatUSDC(market?.totalNoPool)}</span>
      </div>

      {/* Outcome Selection */}
      <div>
        <div className="text-sm font-medium text-white mb-2">Choose Outcome</div>
        ...
        <div>
          ...
          <div className="relative">
            ...
            <button
              ...
              className="absolute right-3 top-1/2 -translate-y-1/2 text-xs text-stadium-green hover:text-white"
            >
              MAX
            </button>
          </div>
